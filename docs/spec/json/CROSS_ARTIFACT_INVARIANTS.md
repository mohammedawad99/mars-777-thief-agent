...
| **INV-10** | The `result` is **self-contained** (K3): the team identities, four GitHub links, per-sub-game commits and scores it carries match the `declaration`/sub-game `log`s for the same `game_uid` — a grader can verify the game **from the result alone**. | SOURCE-SEMANTIC (the report is the binding hand-in) | Ch 9 p.94–96; App E #49/#54 |
...
| **INV-12** | The `result` carries each team's **FastMCP endpoint** (K3); endpoints match those declared in `declaration`. | SOURCE-SEMANTIC (Ch 9 lists FastMCP addresses among the required report contents) | Ch 9 p.94 |
| **INV-13** | The `result` carries each team's **hardware declaration** plus its **keyed-authentication evidence** (`hardware_auth` = the Step-0 `{auth_alg,key_id,auth_tag}`), matching the `declaration`'s Step-0 `step0_auth`. **No key material** crosses any artifact. | SOURCE-SEMANTIC (Ch 9: "cryptographically-signed hardware declarations"); keyed auth = K1/K3 | Ch 9 p.94; Ch 5 p.55–56 |
