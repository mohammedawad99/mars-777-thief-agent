...
| **INV-05** | **[Refined Stage 4E-R13-R1 — same rule, now participant-explicit]** For **each participant `g`**, `declaration.teams.<g>.github_commit` equals `result.sub_games[i].github_commit.<g>` for **every** `i = 1…6`. Ch 5 p.56 permits the code to change **between games**, and the declaration is per-game, so a participant's played commit is **fixed across the six sub-games** of one game and is simply restated in each entry; Ch 9 p.79's "commit id of each sub-game" is satisfied by that restatement. A per-sub-game value differing from the declared one, or from the peer-contributed one, fails result agreement (`E-REPORT-DISAGREE`). | SOURCE-SEMANTIC (played commit declared **and** reported; per-game update) | Ch 5 p.56; App E #53/#54; GIT-003 |
