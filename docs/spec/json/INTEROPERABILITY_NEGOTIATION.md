...
- **NDEC-006 addendum (Stage 4E-R13).** The Event-14 *shape* questions this note left open  are now answered in `RESULT_CONTRACT.md` §R13-4…§R13-6 — one symmetric message per peer  binding `{game_id, game_uid, declaration_ref, result_sha256}`, ordinary operation completion  for the response, and `E-REPORT-DISAGREE`/C-09 on mismatch. The family nevertheless remains  **blocked**, on a deeper question this row's own core definition raises: `sub_games[].tokens`,  `total_tokens` and `sub_games[].github_commit` are **per-team** values that no frozen contract  transmits before the digest is computed, so two peers cannot yet build a byte-identical core.  **Resolved at Stage 4E-R13-R1**: the three participant-owned members become  **participant-scoped `{group_a, group_b}` objects**, each peer sends exactly one  `ResultAgreement` carrying its own `ResultContribution` (six per-sub-game commit + token  values), and the operation's successful response is the receiver's locally computed  `Sha256Digest`. Both peers then build the identical core. **Completed at Stage 4E-R13-R2**:  the request also carries the agreed **`timestamp`**, and the two requests are  **deterministically ordered** — the participant whose `group_id` is byte-wise lower proposes  the timestamp and sends first; the other adopts it verbatim and echoes it in its own single  request.  **`RESULT-APPROVAL-CORE-JOINT-DERIVABILITY: RESOLVED-PROJECT`.**  **NDEC-006's procedure, record shape and exclusions are unchanged, and the NDEC count stays  7.**
