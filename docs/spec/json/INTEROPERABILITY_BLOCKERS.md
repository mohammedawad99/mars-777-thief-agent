...
**One. Stage 4E-R11 resolved the move-rejection shape and narrowed the audit-exchange blocker to a single named cause.**

| Item | Status | Class | Basis |
|---|---|---|---|
| **`AUDIT-EXCHANGE-PAYLOAD` — the interchange representation of the end-of-game audit material** | **Narrowed at Stage 4E-R11 to `BLOCKED-BY-LOG-ARTIFACT-SHAPE`.** Everything around the payload is now exact (`API_BOUNDARIES.md` **O6**): the operation is (f) `submit_audit`, cadence is **one submission per completed sub-game**, producer/storage/transport/consumer ownership is assigned, no verdict or digest is transmitted, no audit ACK and no second audit schema. **The one remaining unknown is the representation**, and its cause is now named rather than open-ended: the natural payload — the finalized per-sub-game log document — is currently classified **LOCAL-ONLY** (*"the persistent log file structure is LOCAL-ONLY"*, and the resolved row above records *"Persistent log = LOCAL-ONLY; only the sealed payload + hash are interop"*, D4/JDEC-007), **and** `LOG_CONTRACT.md` still carries a REVIEW-REQUIRED item on whether ack/reveal are separate `entries[]` events or sub-objects. Promoting a LOCAL-ONLY document to an interoperability contract with an unfrozen nesting is a reviewed decision, not an implementation detail. Of the six candidate representations, a **filesystem path** is rejected outright (not peer-portable) and an **artifact URL** is rejected for now (no stable fetch contract exists); a typed audit projection is rejected as a **second schema** the log already owns. | **BLOCKING** (artifact/log-shape) | Ch 5 §5.4; App E mutual-log-audit row; PRD06-FR-100/101; `LOG_CONTRACT.md` (LOCAL-ONLY + remaining RR); **C-11** |
