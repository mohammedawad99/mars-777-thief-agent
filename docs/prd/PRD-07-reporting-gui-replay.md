...
| **PRD07-FR-085** | `result_sha256` is computed by **PRD-06** over `RESULT_APPROVAL_CORE` and is **non-self-referential**; it is stored in `mutual_agreement.sha256`, with `confirmed` recording agreement state. | PRD06-FR-140…144 |
...
| ID | Requirement | Provenance |
|---|---|---|
| **PRD07-FR-190** | Before sending, each peer **compares the agreed result-approval digest**; both reports must carry the **same** `result_sha256` with `mutual_agreement.confirmed = true`. | **SOURCE-MANDATORY** (LEAGUE-002); NDEC-006 |
