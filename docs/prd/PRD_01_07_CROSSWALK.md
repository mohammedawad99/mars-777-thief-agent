...
|---|---|---|---|
| **CLOSE-F1** | Does the locked declaration already have a series-convention slot? | **NO** — the 16 declaration fields contain no convention/profile slot | **NONE.** No field added, no JDEC created; matrix stays **75** (16/39/9/11). The convention lives in the **negotiation record / profile evidence**, authenticated pre-series and frozen at `CONFIG_LOCKED` |
