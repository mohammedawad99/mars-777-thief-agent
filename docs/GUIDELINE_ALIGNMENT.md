...
| 9.2 | Results-analysis notebook | **PASS** | the guideline's *"or equivalent"*: every statistic lives in tested functions (`research/analysis.py`, `research/stats.py`) and one documented command regenerates every table and figure from the committed rows. A notebook surface on top of those functions is deliberately **not** added while GitHub Actions is a scarce resource, because Jupyter is a large optional dependency whose absence would silently skip the tests that prove the analysis reproduces | Stage 9C, if a notebook surface is wanted |
