...
| `config_<game_id>_g<NN>.json` | Config negotiation, per sub-game | `protocol.config_lock` | **`CONFIG_LOCKED`** | **Yes** (sorted keys, `(",",":")`, UTF-8, NFC, LF, no trailing NL) | `config_sha256` computed over it and stored **outside**; keyed config auth tag in a sidecar | whole series | **Yes**, distinct name per game (App F §2.3–2.4) | **Yes** | none (no secret inside) |
