...
| `series_driver` *(added 6C-C1)* | **The production coordinator for one whole series**: declaration, then per sub-game a config agreement, one fresh `SubGameDriver`, per-round `close_turn`, sub-game closure, and finally the result agreement and persistence. Top-level, not `app/`, because `SeriesRuntime` is - `app/` would need an outward import. Owns no rule, no heuristic, no outcome and no artifact format | series_runtime, app.series_agreements, app.sub_game_driver, app.config_rules, app.pregame_rounds, app.strategy_api | strategy internals, transport, crypto, artifact layout, CLI/process boot | the current sub-game's driver | `E-LOCAL-DEFECT` from the owners it calls |
| `autonomous_boot` *(added 6C-C2)* | **The process's whole life, and no gameplay**: serve, a bounded outbound connection, `SeriesRuntime` construction and `start`, the Step-0 wait, exactly one `SeriesDriver.play_series()`, and a `finally` that stops. Decides no action, outcome, round, config cadence, audit or result | agent_runtime, series_runtime, series_driver, boot_runtimes, startup_budget, infra.artifacts, infra.settings | every gameplay and protocol decision | the series it started | whatever its owners raise; a bounded wait fails closed |
| `startup_budget` *(added 6C-C2)* | The bound and cadence of one process's search for its opponent, and the one retryable type: `E-TRANSPORT` from establishing a session. Numbers are supplied by boot from the locked `watchdog_timeout_sec` | app.protocol_errors, transport.wire_errors | every protocol answer - auth, config, stale, malformed | none (frozen) | re-raises the last failure |
| `boot_runtimes` *(added 6C-C2)* | The per-sub-game evidence/audit factory in production, built from state that already exists: identity from the composition, peer group from the merged declaration, `config_sha256` from the port the lock verifies with | app.outbound_evidence_runtime, app.audit_runtime, protocol.* | any opponent truth | none (closure over series facts) | `ValueError` from the contexts it builds |
| `ingress_release` *(added 6C-C2)* | Putting a served ingress away from any point its startup reached - the one piece `serve`'s failure branch and `stop` need identically | asyncio, socket | everything else | none | only `CancelledError` is swallowed |
| `app.round_opening` *(added 6C-C2)* | **Opening the round the series is on, whether or not it is already open.** Idempotent for the current `gNN`, so an authenticated proposal that arrived before this side opened survives; `gNN+1` still opens fresh through `open_next_round`. Refuses a split round identity and a swapped local candidate | app.pregame_rounds, app.pregame_session_runtime | peer convergence, milestones, locks | none (pure) | `E-LOCAL-DEFECT` |
| `app.audit_milestones` *(added 6C-C2)* | One sub-game audit's single awaitable moment, set **after** the disclosure, outcome and phase are installed. One event covers both arrivals because a disclosure is already refused before the nonce batch | asyncio | every audit fact | the event | none |
| `app.config_rules` *(added 6C-C1)* | **The single projection of a locked `NegotiatedConfig`** into the three things play needs: `SemanticRules`, `TurnLimits` and the opening `LocalTruth`. Replaces `semantic_review.rules_for`, which was **removed**, so review, closure and the live series share one authority | domain.config_model, domain.rules, domain.terminal | negotiation, transport, artifacts | none (pure) | `ValueError` from the domain values it builds |
| `app.pregame_rounds` *(added 6C-C1)* | Opens the next sub-game's config round by `replace`-ing the one field that differs. **Reconstructs nothing** - no key, no authenticator, no digest, no `NegotiatedConfig` field | app.pregame_session_runtime | everything else | none (pure) | `E-LOCAL-DEFECT` on a mismatched round |
| `app.series_agreements` *(added 6C-C1)* | **Sequencing only** for the two boundaries a turn never crosses: the per-round config agreement and the final result agreement. Who opens is `initial_proposer`'s and `is_proposer`'s call; what a lock means is `ConfigLockRuntime`'s; whether an agreement holds is `ResultExchange.is_agreed`'s | app.pregame_session_runtime, app.result_exchange, app.peer_runner, app.config_negotiation_runtime | result data, scores, config state, orchestrator state, artifacts, transport DTOs | none (pure functions) | `E-CONFIG-MISMATCH` / `E-PROTO-STALE` from the owners |
| `app.series_milestones` *(added 6C-C1)* | `PregameMilestones` / `ResultMilestones`: `asyncio.Event` **values** that decide nothing, set by their owner **after** the state moved, so a waiter that wakes finds the fact already true. No polling, no queue, no lock, no global | asyncio | every fact of its own | the events | none |
