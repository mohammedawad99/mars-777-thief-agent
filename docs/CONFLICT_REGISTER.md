...
| **C-15** | Reference config `[email] mode = "draft"` vs the send-only OAuth scope | Ch 9 reference `config/game.toml` example (p.115): `[email] recipient = … ; mode = "draft"` | Appendix E rule 30 + Appendix A §1.3: *use send-only permission for the Gmail interface*, scope `https://www.googleapis.com/auth/gmail.send` and nothing wider | No | A binding Appendix E prohibition outranks an illustrative configuration example | **Resolved: no draft mode.** Creating a Gmail draft requires `gmail.compose` or `gmail.modify`; the `gmail.send` scope cannot create one. So `mode = "draft"` and rule 30 cannot both be satisfied, and the binding rule wins. The agent sends. `mode` is not implemented, not read and not offered as a setting. | High - the scope requirement is stated twice (rule 30 and Appendix A §1.3) and `mode` appears exactly once, in an example, with no normative text anywhere. | `infra/gmail_credentials.py` refuses a token file granting any scope other than `gmail.send`; no draft path exists. Operators who want a non-sending rehearsal use the gated live smoke against their **own** address instead. | Recorded at Stage 9A-2C |
...
Conflict Register is the single authority for `C-01…C-15`.
...
| **Status** | **COMPLETE at Stage 5-R8.** (a) and (b) shipped with the live capture checkpoint; (c) shipped with the semantic-audit checkpoint: each side retains the real capture transcript in both directions (`app/capture_transcript.py`), discloses its own half inside the existing `AuditDocument`, refuses a disclosure that differs row for row from what it observed, and then **replays the whole sub-game** from the two disclosed logs (`app/semantic_replay.py`, `app/semantic_capture.py`, `app/semantic_review.py`). Trajectories are reconstructed from the config-locked start cells, actions are judged by `domain.rules` / `domain.barriers`, the public barrier chronology is rebuilt rather than accepted, and every capture answer is recomputed against the cell its answerer later disclosed. |
| **Sanctions** | Every mapping in this row is the source's own word for that fault; none is invented, and no new score, outcome or error identity exists. *(One later addition is not source-worded and says so explicitly: `DISHONEST_SCENT_EMISSION`, added by **JDEC-018** at Stage 5-R8 Part 2A-R1, is **PROJECT-DERIVED** — it reuses the technical-loss lifecycle below but the lecturer names neither the verdict nor a sanction for that case. It creates **no C-15**: the source under-specifies the case rather than contradicting itself.)* **Scored, not disqualifying** (sub-game ends `Outcome.TECHNICAL_LOSS`, which `domain.scoring` already scores **0/0** — Ch 3 Table 2, App E #48, C-07 — and the series audit stays verified): an **illegal move** (`GAME-003` / E-13, *"Illegal move → technical loss"*), an **illegal placement** (`BAR-004`, *"illegal placement → audit loss"*), a **false capture declaration** (`CRYPTO-005` / E-22, *"Score 0 + technical loss, no appeal"*), and — from Part 2B — a **physically dishonest scent emission** (JDEC-018; scored, and deliberately **not** `TAMPERING`, because scent sits outside the sealed eight and every hash can verify). **Disqualifying** (`SeriesAuditGate` records `TAMPERED`, result agreement is blocked): a **dishonest capture answer** (`CRYPTO-004` / E-21, *"Immediate DQ for denying reality"*) and a **disclosure that cannot be reconciled with facts the peer independently holds** — a start cell that is not the locked one, a trajectory that contradicts the mover's own earlier sealed state, or a barrier set that contradicts the placements both sides publicly revealed — which is `CRYPTO-003` / `REPLAY-002` applied to a forgery that happens to hash correctly. **Both at once**: a false declaration the opponent affirmed as `CAUGHT` is `FALSE_CLAIM_AFFIRMED` — scored 0/0 for the claimant *and* disqualifying for the answerer, naming both sides, and it can never be recorded as a capture. |
| **Not conflated** | An honestly committed **illegal action** is deliberately **not** reported as a hash mismatch or as transcript fabrication. `H_commit`, the nonce, the disclosed sealed material and the capture transcript can all be correct while the action they truthfully record was against the rules; the audit then reports `FinalAuditVerdict.VERIFIED_OK` with a semantic finding of `ILLEGAL_ACTION`, and only the end event changes. |
...
## C-14 — Full scent-model agreement vs the three-scalar config (Stage 5-R8)

| Field | Content |
|---|---|
| **Conflict** | `SCENT-001` requires the agreed **scent emission+decay model** to be cryptographically locked before the series, and `SCENT-003` requires the **full** emission/decay model plus a concrete numeric example to be exchanged and verified as identically interpreted first. The only scent material the project could lock was the three Appendix-F scalars carried by `pheromones` inside the 35-member `NegotiatedConfig` — `center_intensity` 0.9, `decay` 0.10, `field_size` 5 (App F Table 16). |
| **Confirmed?** | **Yes — a real coverage gap in the derived contract, not a source contradiction.** Those three scalars do not determine the model: they fix neither the 25 radial weights that turn a centre intensity into a field, nor the worked numbers that prove two peers read the recurrence the same way, nor the C-10 saturation choice. Two peers could therefore agree a byte-identical `config_sha256`, lock it, and still emit and decay scent differently — which is exactly the deviation `SCENT-001` calls game-voiding. A profile token would not have closed it either: a token names an interpretation, it does not carry the numbers. |
| **Authority** | Source requires the agreement (`SCENT-001`, `SCENT-003`, App E #23). The **representation** of a scent model was never fixed by the book, so the carrier, the canonical bytes and the digest are PROJECT-CONTRACT, chosen under the academic-freedom rule (PDF p.5 / book v). |
| **Resolution** | A separate semantic `ScentModelAgreement` (model id, the three FIXED values, all 25 kernel weights, the worked examples) exists **beside** `NegotiatedConfig` and is exchanged in full through the **existing** `ConfigProposal` family. Before the lock, negotiation requires exact agreement read three independent ways — values, canonical rendering and independently derived digest. `scent_model_sha256` is an unkeyed content digest of that model and is a member of `ConfigLockContext`, so the **existing** keyed config-auth proof authenticates it with no framing change. `g01` freezes the successfully locked identity for the whole series and `g02…g06` must present the same one. Each `config_<game_id>_g<NN>.json` persists the actual agreed model, its digest, the lock context and the safe auth evidence. Frozen by **JDEC-017**. |
| **Not C-10** | **C-10 is a different conflict and is unchanged.** C-10 is the *recurrence* conflict — the stated state domain `[0, 0.9]` against an update equation written with a lower clamp only — and its resolution is the saturating evolution in `domain.scent`. C-14 is about *which complete model two peers agreed on and locked*. C-14 consumes C-10's resolution as one member of the agreed model; it does not restate, replace or renumber it. |
| **Impact** | **No fifth artifact family** — the evidence lives in the existing per-sub-game config artifact. **No change to the 35-member `NegotiatedConfig`** and **no change to what `config_sha256` covers**: the model is bound by its own separate digest, and the two identities are never merged. **No new peer family, wire kind or FastMCP tool** for the pre-game exchange, and no new error identity: a missing or valid-but-different model is the existing `E-CONFIG-MISMATCH`, a malformed one the existing malformed-message path. Registers unchanged: families 8, kinds 9, tools 4, ports 21, errors 22, requirements 91. |
| **Status** | **RESOLVED for the pre-game/series contract — Stage 5-R8.** Implemented and CI-green across the scent domain, model foundation, proposal codec, strict agreement, crypto lock, series freeze and artifact evidence checkpoints. **This resolution covers the agreement and the lock only.** Live emission during gameplay, receiving or consuming the opponent's scent, live scent transport and a scent audit are **not** part of C-14's resolution and remain open for Reveal V2 / live distributed scent. |

## C-15 — Repository sibling-isolation rule vs the C2-A symmetric counted implementation (Stage C2-A)

| Field | Content |
|---|---|
| **Date** | 2026-08-23 |
| **Stage** | C2-A — counted interoperability implementation (Step-0 V2, role-specific commit authority) |
| **Conflict** | The repository-local operating contract (`CLAUDE.md`) instructs each agent to operate **only** inside its own repository and to **never** inspect or modify the sibling repository. The frozen C2-A counted contract requires the *same* protocol surfaces — the shared game contract, `DeclarationWire` V2, the Step-0 authenticated core, the slot rule and the role-aware commit validation — to be implemented **symmetrically** in both Police and Thief, and to be proven symmetric before release. Under strict sibling isolation that symmetry cannot be implemented or verified in one stage. |
| **Confirmed?** | **Yes — a real governance tension, not a source contradiction.** Neither instruction is wrong: repository independence is what keeps the two agents genuinely separate implementations, while a counted series is played by *both* of this group's repositories against one opponent and therefore needs one identical wire contract on both sides. |
| **Authority** | The sibling-isolation rule is a PROJECT-CONTRACT working discipline, not a book or Appendix-E requirement. The operator is the authority over it and issued an explicit, stage-scoped ruling. |
| **Ruling** | The explicit C2-A operator instruction authorising symmetric implementation across both repositories is a **deliberate, stage-scoped override** of the sibling-isolation rule. `CLAUDE.md` is **not** rewritten; the override does not extend beyond C2-A. |
| **Allowed scope** | Inspecting and modifying both repositories **only** for the frozen counted interoperability surfaces and their tests, docs and configuration. |
| **Preserved isolation constraints** | No cross-repository imports. No symlinks between repositories. Neither repo reads the other's runtime files. No shared mutable runtime state. No sharing or copying of real secrets. No build- or run-time dependency on the sibling. No strategy coupling between Police and Thief. Each repository must remain independently installable, testable, buildable, runnable and releasable, with independent commits and pushes. |
| **Impact** | Cross-repository comparison is used **only** to prove semantic symmetry; every change is implemented, tested, built, committed and pushed independently in each repository. |
| **Status** | **RESOLVED for C2-A by explicit operator ruling.** The override expires with the stage; repository independence remains mandatory. |
