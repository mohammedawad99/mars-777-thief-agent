...
- **Stage 3C - Local application / turn orchestration foundation.** Next authorized stage; **not started**. Planned: consume the validated deterministic domain primitives; a local turn command/application service; explicit action choice (move **or** barrier); local state ownership; deterministic transition/result objects. **No** public networking and **no** cryptography yet.
