...
| **Gmail reporting** (`REPORT-001`, `REPORT-002`, `JSON-001`, `JSON-002`) | `IMPLEMENTATION_COMPLETE` / `LIVE_SEND_NOT_PERFORMED` | `uv run python -m mars777_thief.report_main --result …`. The agreed result artifact is attached as JSON to the fixed Appendix F Table 20 address, never sent as free text; eligibility is the Appendix E rule 35 mutual agreement. Proved against a fake provider at the adapter seam; **no real message has been sent**. `docs/reference/REPORTING.md` |
